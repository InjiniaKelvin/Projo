...
async function cleanupTestUsers() {
  try {
    ...
    if (userIds.length > 0) {
      ...
      console.log(`✅ Deleted ${techBookingsDeleted.deletedCount} technician bookings`);
    }

    if (walletIds.length > 0) {
      const walletsDeleted = await Wallet.deleteMany({ _id: { $in: walletIds } });
      console.log(`✅ Deleted ${walletsDeleted.deletedCount} wallets`);
    }
    ...
    console.log('✅ Cleanup complete! Ready to run E2E tests.');
