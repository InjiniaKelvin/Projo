...
if echo "$response" | grep -q "success.*true"; then
 echo -e "${GREEN}[OK] PASS${NC} - Backend is running on port 5000"
 echo " Response: $response"
else
 echo -e "${RED}[FAIL] FAIL${NC} - Backend is not running on port 5000"
...
if echo "$response" | grep -q "database.*connected"; then
 echo -e "${GREEN}[OK] PASS${NC} - MongoDB Atlas is connected"
else
 echo -e "${RED}[FAIL] FAIL${NC} - MongoDB is not connected"
 exit 1
fi
echo ""
...
if echo "$route_response" | grep -q "Validation failed\|required"; then
 echo -e "${GREEN}[OK] PASS${NC} - Booking route is accessible and validates input"
else
 echo -e "${RED}[FAIL] FAIL${NC} - Booking route is not responding correctly"
...
if echo "$booking_response" | grep -q "success.*true"; then
 echo -e "${GREEN}[OK] PASS${NC} - Booking submitted successfully"
 booking_id=$(echo "$booking_response" | grep -o '"bookingId":"[^"]*"' | cut -d'"' -f4)
 echo " Booking ID: $booking_id"
 echo " Full Response: $booking_response"
else
 echo -e "${RED}[FAIL] FAIL${NC} - Booking submission failed"
...
if [ -f "app/booking/details.tsx" ]; then
 ...
 echo -e "${GREEN}[OK] PASS${NC} - File exists with debug logging ($log_count log statements)"
 else
 echo -e "${YELLOW}[WARNING] WARN${NC} - File exists but may have few debug logs"
 fi
else
 echo -e "${RED}[FAIL] FAIL${NC} - booking/details.tsx file not found"
fi
echo ""
...
if grep -q "localhost:5000" app/booking/details.tsx; then
 echo -e "${GREEN}[OK] PASS${NC} - API endpoint uses correct port (5000)"
else
 echo -e "${RED}[FAIL] FAIL${NC} - API endpoint not using port 5000"
fi

# Check for Authorization header
if grep -q "Authorization" app/booking/details.tsx; then
 echo -e "${GREEN}[OK] PASS${NC} - Authorization header is present"
else
 echo -e "${RED}[FAIL] FAIL${NC} - Missing Authorization header"
fi

# Check for useEffect dependencies
if grep -q "user?.firstName, user?.lastName" app/booking/details.tsx; then
 echo -e "${GREEN}[OK] PASS${NC} - useEffect has proper dependency array"
else
 echo -e "${YELLOW}[WARNING] WARN${NC} - useEffect dependencies may need review"
fi

# Check for storage helper
if grep -q "const storage" app/booking/details.tsx; then
 echo -e "${GREEN}[OK] PASS${NC} - Storage helper is present"
else
 echo -e "${RED}[FAIL] FAIL${NC} - Missing storage helper"
fi
...
echo "1. [OK] API endpoint changed to port 5000"
echo "2. [OK] Authorization header added"
echo "3. [OK] useEffect infinite loop fixed"
echo "4. [OK] Text node errors resolved"
echo "5. [OK] Storage helper added"
echo ""
echo "Next Steps:"
