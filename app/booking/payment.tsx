...
export default function PaymentScreen() {
 ...
 await pollPaymentStatus(mpesaResult.transaction._id);
 } else {
 ...
 // In a real app, open the checkout URL in a web view or browser
 Alert.alert(
 'Card Payment',
 ...
 text: 'Continue',
 onPress: () => {
 // Open checkout URL (implement web view or browser)
 ...
 throw new Error(cardResult.message || 'Card payment failed');
 }
 break;

 case 'wallet':
 // Wallet payment - transfer to escrow
 response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/payments/escrow/deposit`, {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 'Authorization': `Bearer ${await getAuthToken()}`
 ...
 amount: amount,
 bookingId: bookingId,
 description: `Escrow for ${booking?.serviceName || 'service'}`
 })
 });
 ...
 let attempts = 0;
 const maxAttempts = 20; // Poll for 2 minutes (6 seconds * 20)
 
 const poll = async () => {
 try {
 const response = await fetch(
 `${process.env.EXPO_PUBLIC_API_URL}/api/payments/status/${transactionId}`,
 {
 headers: {
 'Authorization': `Bearer ${await getAuthToken()}`
 }
 ...
 Alert.alert(
 'Payment Pending',
 'Your payment is taking longer than expected. Please check your booking status later.'
 );
 router.back();
 }
 } catch (error) {
 ...
 setPaymentStatus('failed');
 Alert.alert('Payment Error', 'Failed to verify payment status.');
 }
 };
