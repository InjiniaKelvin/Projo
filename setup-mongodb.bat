...
echo [LAUNCH] Setting up MongoDB for QuickFix App...
echo.

:: Check if MongoDB is already installed
echo [CHECKLIST] Checking MongoDB installation...
...
if %errorlevel% == 0 (
 ...
 echo [FAIL] MongoDB not found. Please install MongoDB Community Server.
...
if %errorlevel% == 0 (
 ...
 echo [WARNING] MongoDB service not found. Starting MongoDB manually...
...
if not exist "data\db" (
 ...
 mkdir data\db
 echo [OK] Data directory created: %cd%\data\db
 echo.
)
...
if not exist ".env" (
 ...
 echo [NOTE] Creating .env file with default settings...
 (
...
if /i "%seed_choice%"=="y" (
 ...
 echo [FAIL] Database seeding failed
 )
 echo.
)
...
:: Start the server
echo [LAUNCH] Do you want to start the development server now?
...
if /i "%start_choice%"=="y" (
 ...
 echo.
 echo [OK] MongoDB setup completed successfully!
 echo.
 echo [LAUNCH] To start the server manually, run:
 echo npm run server:dev
 echo.
 echo [DOCS] Useful commands:
